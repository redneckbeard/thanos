...
func init() {
	...
	ObjectType.Def("class", MethodSpec{
		...
		TransformAST: func(rcvr TypeExpr, args []TypeExpr, blk *Block, it bst.IdentTracker) Transform {
			// Compile to the package-level metaclass var: ChangeClass, etc.
			// The var name is ClassName + "Class" and is generated by the compiler.
			className := rcvr.Type.ClassName()
			if className == "" {
				// Fallback for non-class types — shouldn't normally happen
				return Transform{Expr: it.Get("nil")}
			}
			return Transform{
				Expr: it.Get(className + "Class"),
			}
		},
	})
